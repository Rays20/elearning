...
function validateDate(date) {
    // Adjust the regular expression for the format yyyy/mm/dd
    var dobRegex = /^(\d{4})\/(\d{1,2})\/(\d{1,2})$/;

    if (!dobRegex.test(date)) {
        return 'Invalid date format';
    }

    var [, year, month, day] = date.match(dobRegex);
    day = parseInt(day, 10);
    ...
    if ([4, 6, 9, 11].includes(month) && day > 30) {
        return 'Invalid day (this month contains 30 days)';
    }
    ...
    if ((month === 2 && (day > 29 || (day > 28 && !((year % 4 === 0 && year % 100 !== 0) || year % 400 === 0))))) {
        return 'Invalid day (February logic)';
    }
